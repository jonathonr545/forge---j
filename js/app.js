...
(function() {
  ...
  const state = {
    currentTab: 'home',
    cycleIndex: 0,          // 0-6 position in the 7-day cycle
    workoutActive: false,
...
Store.saveActiveWorkout({
    ...
    el.innerHTML = `
      ...
      <div class="today-card ${typeClass}">
        <div class="today-card-top">
          ...
          <div class="today-day-badge">Day ${state.cycleIndex + 1}/7</div>
        </div>
      </div>
  ...
  function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (isToday && completed && completed.type !== 'rest') {
        ...
        const futureIdx = ((state.cycleIndex + daysAhead) % 7 + 7) % 7;
  ...
  function renderComplete(el) {
    const day = FORGE_DATA.cycleDays[(state.cycleIndex + 6) % 7]; // previous day since we already advanced
  ...
  function advanceCycle() {
    state.cycleIndex = (state.cycleIndex + 1) % 7;
  ...
  function renderSettings(el) {
    ...
    el.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div class="warmup-card">
          <label style="font-size:13px;color:var(--text-secondary);display:block;margin-bottom:4px;">Current cycle day (1-7)</label>
          <input type="number" class="weight-input" id="settings-cycle" value="${state.cycleIndex + 1}" min="1" max="7" inputmode="numeric" style="width:100%;">
        </div>
  ...
  function saveSettings() {
    ...
    state.bodyWeight = bw;
    state.cycleIndex = Math.max(0, Math.min(6, cycle - 1));
  ...
  function clearToday() {
    ...
    if (confirm('Clear today\'s workout? This removes the log and rolls back your cycle position.')) {
      ...
      state.cycleIndex = (state.cycleIndex + 6) % 7;
      const settings = Store.getSettings();
      settings.cycleIndex = state.cycleIndex;
      Store.saveSettings(settings);
